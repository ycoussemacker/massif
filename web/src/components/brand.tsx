/** Partner brand marks (Strava / Garmin) + the sober "→ Strava" deep-link.
 *  Shared, server-compatible (no client hooks) so both the dashboard (server component)
 *  and the Connections panel ("use client") can import the same single source of truth.
 *  Official brand paths from Simple Icons, inlined (no external asset / CSP issue); fill=currentColor. */
...
/** Sober deep-link to the original Strava activity. Muted (stone) at rest so it never competes with
 *  the load numbers; the brand mark + label + arrow tint to the partner orange on hover. Renders
 *  nothing when the activity didn't come from Strava (or has no id). */
export function StravaLink({
  ...
  if (source !== "strava" || !sourceActivityId) return null;
  return (
    <a
      href={`https://www.strava.com/activities/${sourceActivityId}`}
      target="_blank"
      rel="noreferrer"
      title="Ouvrir sur Strava"
      className={`group inline-flex items-center gap-1 text-xs font-medium text-stone-400 transition-colors hover:text-strava dark:text-stone-500 dark:hover:text-strava ${className}`}
    >
      <StravaLogo className="h-3 w-3 shrink-0" />
      <span>Strava</span>
      <span aria-hidden className="text-[0.85em] transition-transform group-hover:translate-x-px group-hover:-translate-y-px">↗</span>
    </a>
  );
}
