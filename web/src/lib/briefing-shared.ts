 ...
 *  MIRROR of coach/src/briefing-shared.ts — keep the SYSTEM text, the schema, and buildForwardPlanRows
 *  IDENTICAL across the two (web/ and coach/ are separate workspaces, no cross-import, same discipline as
 *  load.ts ↔ load.py). */
...
CHAMP why : UNE SEULE PHRASE — la raison principale, la plus importante, de la reco d'aujourd'hui (≤ ~25 mots,
...
la LONGUEUR des champs (why = 1 phrase, state_assessment = 2-3 phrases).
 ...
 *  and the web path call it. MIRROR in coach/src/briefing-shared.ts. */
